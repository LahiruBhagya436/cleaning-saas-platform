...
const DAY_OPTS: [string, number][] = [['Mon', 1], ['Tue', 2], ['Wed', 3], ['Thu', 4], ['Fri', 5], ['Sat', 6], ['Sun', 0]]
...
export default function WorkerHomePage() {
  ...
  const addDay = async () => {
    if (!wDate) { toast.error('Choose a date.'); return }
    setSavingDay(true)
    try {
      await staffApi.addMyDay({ workDate: wDate, startTime: wStart, endTime: wEnd, isAvailable: true })
      toast.success('Day saved.'); setWDate(''); setDayOpen(false); load()
    } catch (e: any) { toast.error(e?.message ?? 'Could not save.') } finally { setSavingDay(false) }
  }
  ...
  const addRange = async () => {
    ...
    try {
      ...
      toast.success(`${res.data?.count ?? 0} days added.`); setRStart(''); setREnd(''); setRangeOpen(false); load()
    } catch (e: any) { toast.error(e?.message ?? 'Could not save.') } finally { setSavingRange(false) }
  }
    ...
    <div className="space-y-8">
      ...
      <section>
        <h2 className="font-display text-xl text-neutral-900 mb-3 flex items-center gap-2">
          <Briefcase size={18} className="text-brand-600" /> Today's jobs
        </h2>
        {jobs.length === 0 ? (
          <div className="bg-white border border-dashed border-neutral-200 rounded-xl p-6 text-center text-sm text-neutral-400">No jobs today.</div>
        ) : (
            ...
            {jobs.map((j) => (
              <div key={j.id} className="bg-white border border-neutral-200 rounded-xl p-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-neutral-800 truncate">{j.customer?.fullName ?? 'Customer'}</p>
      ...
      <section>
        <h2 className="font-display text-xl text-neutral-900 mb-1 flex items-center gap-2">
          <CalendarDays size={18} className="text-teal-600" /> My availability
        </h2>
        <p className="text-sm text-neutral-500 mb-3">Add the days and times you can work.</p>

        <div className="flex flex-wrap gap-2 mb-4">
          <Button size="sm" onClick={() => { setDayOpen((v) => !v); setRangeOpen(false) }}>
            {dayOpen ? <X size={14} /> : <Plus size={14} />}{dayOpen ? 'Cancel' : 'Add day'}
          </Button>
          <Button size="sm" variant="outline" onClick={() => { setRangeOpen((v) => !v); setDayOpen(false) }}>
            <CalendarDays size={14} />{rangeOpen ? 'Cancel range' : 'Date range'}
          </Button>
        </div>
        ...
        {dayOpen && (
          <div className="bg-white border border-neutral-200 rounded-xl p-3 mb-4 flex flex-wrap items-end gap-2">
            ...
            <Button size="sm" onClick={addDay} loading={savingDay}>Save day</Button>
          </div>
        )}
        ...
        {rangeOpen && (
          <div className="bg-white border border-neutral-200 rounded-xl p-3 mb-4 space-y-3">
            <div className="flex flex-wrap items-end gap-2">
              <div><label className="block text-xs text-neutral-500 mb-0.5">From</label>
              ...
              <div><label className="block text-xs text-neutral-500 mb-0.5">To</label>
              ...
              <div><label className="block text-xs text-neutral-500 mb-0.5">End</label>
            ...
            <Button size="sm" onClick={addRange} loading={savingRange}>Save range</Button>
          </div>
        )}

        {schedule.length === 0 ? (
          <div className="bg-white border border-dashed border-neutral-200 rounded-xl p-6 text-center text-sm text-neutral-400">No availability added yet.</div>
        ) : (
            ...
            {schedule.map((s) => (
              <div key={s.id} className={cn('bg-white border rounded-xl px-3 py-2.5 flex items-center gap-3', s.isAvailable ? 'border-neutral-200' : 'border-neutral-100 opacity-60')}>
                <span className="w-10 h-10 rounded-lg bg-teal-50 text-teal-700 flex flex-col items-center justify-center leading-none shrink-0">
                  <span className="text-[9px] uppercase">{format(new Date(s.workDate), 'MMM', { locale: enGB })}</span>
                  <span className="text-sm font-bold">{format(new Date(s.workDate), 'd')}</span>
                </span>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-neutral-800 capitalize">{format(new Date(s.workDate), 'EEEE', { locale: enGB })}</p>
