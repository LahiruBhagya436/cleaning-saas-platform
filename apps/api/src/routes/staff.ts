...
staffRoutes.get('/', requireAdmin, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const staff = await prisma.user.findMany({
      where:   { role: 'staff', isActive: true, companyId: req.user!.companyId },
...
staffRoutes.get('/me/jobs/today', requireStaff, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const jobs = await prisma.booking.findMany({
      where:   { staffId: req.user!.userId, companyId: req.user!.companyId, scheduledAt: { gte: todayStart, lte: todayEnd }, status: { in: ['confirmed', 'in_progress'] } },
...
staffRoutes.post('/me/jobs/:id/start', requireStaff, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const booking = await prisma.booking.findFirst({ where: { id: req.params.id, staffId: req.user!.userId, status: 'confirmed', companyId: req.user!.companyId } })
